...
use crate::ty::*;
use crate::{
    DescriptorBinding, EntryPoint, ExecutionModel, Instr, Instrs, Location, Manifest,
    ResourceLocator, SpirvBinary,
};
...
impl<'a> ReflectIntermediate<'a> {
    ...
    fn resolve_ref(&self, ty_id: TypeId) -> Option<(TypeId, &Type)> {
        self.ptr_map
            .get(&ty_id)
            .and_then(|ty_id| self.ty_map.get(ty_id).map(|ty| (*ty_id, ty)))
    }
    ...
    fn get_var_desc_bind_or_default(&self, var_id: VariableId) -> DescriptorBinding {
        let desc_set = self
            .get_deco_u32(var_id, None, Decoration::DescriptorSet)
            .unwrap_or(0);
        let bind_point = self
            .get_deco_u32(var_id, None, Decoration::Binding)
            .unwrap_or(0);
        DescriptorBinding::desc_bind(desc_set, bind_point)
    }
    fn get_name(&self, id: InstrId, member_idx: Option<u32>) -> Option<&'a str> {
        self.name_map.get(&(id, member_idx)).map(|x| *x)
    }
        ...
        let (key, value) = match instr.opcode() {
            ...
            OP_TYPE_IMAGE => {
                ...
                let img_ty = if op.dim == Dim::DimSubpassData {
                    ...
                    // have given format.
                    let unit_fmt =
                        ImageUnitFormat::from_spv_def(op.is_sampled, op.is_depth, op.color_fmt)?;
                    let arng =
                        ImageArrangement::from_spv_def(op.dim, op.is_array, op.is_multisampled)?;
            ...
            OP_TYPE_ARRAY => {
                ...
                let nrepeat = self
                    ...
                    .ok_or(Error::CorruptedSpirv)?;
                let stride = self
                    .get_deco_u32(op.ty_id, None, Decoration::ArrayStride)
                    .map(|x| x as usize);
            ...
            OP_TYPE_STRUCT => {
                ...
                for (i, &member_ty_id) in op.member_ty_ids.iter().enumerate() {
                    ...
                    if let Type::Matrix(ref mut mat_ty) = proto_ty {
                        let mat_stride = self
                            .get_deco_u32(op.ty_id, Some(i), Decoration::MatrixStride)
                            .map(|x| x as usize)
                            .ok_or(Error::CorruptedSpirv)?;
                    ...
                    if let Some(offset) = self
                        .get_deco_u32(op.ty_id, Some(i), Decoration::Offset)
                        .map(|x| x as usize)
                    {
    ...
    fn populate_one_var(&mut self, instr: &Instr<'a>) -> Result<()> {
        fn ty2buf(ty: &Type) -> Option<(u32, Type)> {
            match ty {
                Type::Array(arr_ty) => {
                    if let Type::Struct(struct_ty) = &*arr_ty.proto_ty() {
        ...
        let (ty_id, ty) = if let Some(x) = self.resolve_ref(op.ty_id) {
            x
        } else {
            ...
            StorageClass::UniformConstant => {
                ...
                let desc_ty = if let Type::Image(_) = ty {
                    ...
                    let input_attm_idx = self
                        .get_deco_u32(op.alloc_id, None, Decoration::InputAttachmentIndex)
                        .ok_or(Error::CorruptedSpirv)?;
            ...
            while let Some(instr) = instrs.peek() {
                match instr.opcode() {
                    ...
                    OP_LOAD => {
                        ...
                        if let Some(&x) = access_chain_map.get(&rsc_id) {
                            rsc_id = x
                        }
                    ...
                    OP_ACCESS_CHAIN => {
                        ...
                        if access_chain_map
                            .insert(op.rsc_id, op.accessed_rsc_id)
                            .is_some()
                        {
                            return Err(Error::CorruptedSpirv);
                        }
            ...
            for accessed_var_id in accessed_var_ids {
                ...
                let collision = match accessed_var {
                    Variable::Input(location, ivar_ty) => {
                        let mut collision = entry_point
                            .manifest
                            .input_map
                            .insert(location, ivar_ty)
                            .is_some();
                        if let Some(name) = self.get_name(accessed_var_id, None) {
                            collision |= entry_point
                                .manifest
                                .var_name_map
                                .insert(name.to_owned(), ResourceLocator::Input(location))
                                .is_some();
                        }
                        collision
                    }
                    Variable::Output(location, ivar_ty) => {
                        ...
                        if let Some(name) = self.get_name(accessed_var_id, None) {
                            collision |= entry_point
                                .manifest
                                .var_name_map
                                .insert(name.to_owned(), ResourceLocator::Output(location))
                                .is_some();
                        }
                        collision
                    }
                    Variable::Descriptor(desc_bind, desc_ty) => {
                        let mut collision = entry_point
                            .manifest
                            .desc_map
                            .insert(desc_bind, desc_ty)
                            .is_some();
                        if let Some(name) = self.get_name(accessed_var_id, None) {
                            collision |= entry_point
                                .manifest
                                .var_name_map
                                .insert(name.to_owned(), ResourceLocator::Descriptor(desc_bind))
                                .is_some();
                        }
                        collision
                    }
